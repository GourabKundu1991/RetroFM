...
const InsuranceDocumentsScreen = ({ navigation }) => {
    ...
    const getAllData = (filterYear) => {
        AsyncStorage.getItem('userToken').then(val => {
            if (val != null) {
                ...
                apiClient
                    .post(`${BASE_URL}/get_insurance_list`, formdata, {
                        headers: {
                            'Content-Type': 'multipart/form-data',
                            accesstoken: `${AccessToken}`,
                            useraccesstoken: JSON.parse(val).token
                    ...
                    .then((responseJson) => {
                        console.log("get_insurance_list:", responseJson.data);
                        if (responseJson.data.bstatus == 1) {
                            setLoading(false);
                            setFromList(responseJson.data.insurance);
                            if (responseJson.data.insurance.length == 0) {
                    ...
                    .catch((error) => {
                        setLoading(false);
                        console.log("get_insurance_list Error:", error);
                        Toast.show({ description: t("Sorry! Somthing went Wrong. Maybe Network request Failed") });
                    });
            } else {
                setLoading(false);
            ...
            <Box flex={1} bg={"#F1F1F1"}>
                {/* <HStack style={{ height: 60 }} bg={colorTheme.normal} justifyContent="space-between" alignItems="center" paddingX="4" paddingY="3" space={2}>
                    <HStack alignItems={'center'} space={8}>
                        ...
                        <Text color="#ffffff" fontSize="16" textAlign="center" fontWeight="bold">{t("Insurance Documents")}</Text>
                    </HStack>
                </HStack> */}
                <ScrollView>
                    <Box padding="3">
                        <VStack>
                            <VStack style={styles.productbox} space={1}>
                                <Text color={"#111111"} fontSize="md" >{t("Financial Year")}</Text>
                                    ...
                                    {fromList.map((item, index) =>
                                        <Box key={index} style={styles.productbox}>
                                            ...
                                            <HStack justifyContent="space-between" alignItems="center" w="100%" padding={2}>
                                                <Text color="#666666" fontSize="xs" textAlign="center" fontWeight="medium" textTransform="capitalize">{t("Document")} {index + 1}:</Text>
                                                <Icon name="document-attach-outline" size={36} color="#111111" />
                                            </HStack>
                                            <HStack justifyContent="space-between" alignItems="center" w="100%" padding={2}>
                                                <Button size="sm" backgroundColor={'#ffffff'} borderColor={colorTheme.normal} style={styles.custbtn} variant="outline" _text={{ color: colorTheme.normal, fontWeight: 'bold', fontSize: 13 }} onPress={() => onView(item.docs)}>{t("View Document")}</Button>
...
const styles = StyleSheet.create({
    ...
    custbtn: { width: '48%', borderRadius: 7, overflow: 'hidden' },
    productbox: { borderRadius: 7, width: '96%', margin: '2%', backgroundColor: '#ffffff', padding: 15, overflow: 'hidden' },
