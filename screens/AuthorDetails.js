    ...
    const getAuthor = () => {
        AsyncStorage.getItem('userToken').then(val => {
            if (val != null) {
                ...
                formdata.append("authorId", route.params.authorId);
                apiClient
                    .post(`${BASE_URL}/get-authors`, formdata, {
                        headers: {
                    ...
                    .then((responseJson) => {
                        console.log("Author Details:", responseJson);
                        if (responseJson.status == true) {
                            setAuthorDetails(responseJson.details[0]);
                            setStoryList(responseJson.details[0].stories);
                            setLoading(false);
                        } else {
                            setLoading(false);
                    ...
                    <ScrollView style={{ width: "100%" }} showsVerticalScrollIndicator={false}>
                        ...
                        <HStack paddingHorizontal={10} marginTop={-30} space={3}>
                            <Avatar
                                bgColor={"#c90c16"}
                                size={100} p={1}
                                source={{ uri: authorDetails.image }}
                            />
                            <VStack marginTop={39}>
                                <Text color={"#ffffff"} fontSize="md">{authorDetails.name}</Text>
                                <Text color={"#c90c16"} fontSize="xs">{authorDetails.title}</Text>
                                <HStack space={2}>
                                    <View width={80} height={38} justifyContent={'center'} borderRadius={10} backgroundColor={'#333333'} style={{ paddingVertical: 1, paddingHorizontal: 8 }}>
                                        <Text color={"#ffffff"} fontSize="sm" lineHeight={14}>{authorDetails.plays} <Text color={"#888888"} fontSize="xs">Plays</Text></Text>
                                    </View>
                                    <View width={80} height={38} justifyContent={'center'} borderRadius={10} backgroundColor={'#333333'} style={{ paddingVertical: 1, paddingHorizontal: 8 }}>
                                        <Text color={"#ffffff"} fontSize="sm" lineHeight={14}>{authorDetails.followers} <Text color={"#888888"} fontSize="xs">Followers</Text></Text>
                                    </View>
